#!/bin/bash
# PEAD Engine — runs Mon-Fri 4:00 PM IST (10:30 UTC) from VPS cron
set -euo pipefail
...
LOG="/root/artha-dashboard/logs/pead_$(date +%Y%m%d).log"
mkdir -p "$(dirname "$LOG")"

echo "=== PEAD run $(date) ===" >> "$LOG"
cd /root/artha-dashboard
python3 scripts/pead_engine.py >> "$LOG" 2>&1
echo "=== done $(date) ===" >> "$LOG"
